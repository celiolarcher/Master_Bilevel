	  ...
	  if(this==this->UP){
		...
		DELW->resetPopulation();
	      ...
	      if(sol->feasible){
		    ...
		    for(int i=0;i<1e5 && fabs((DELW->best->upLevelFunction-ValueReff)/(fabs(ValueReff)+1))>1e-2;i++){      
		    //for(int i=0;i<100;i++){
			...
			for(int p=0;p<popSizeLW;p++)
			    DELW->recombinePopulation(0.6,p);
			
			for(int p=0;p<popSizeLW;p++)
			    DELW->selectPopulation(p);
		    }    
		    
		    if(fabs((DELW->best->upLevelFunction-ValueReff)/(fabs(ValueReff)+1))>1e-2)  DELW->best->feasible=0;
		}
